...
from da_hf5_dz.config import OLD_DZ_RULE, AggregationIdentifiers, filepath_shared_folder, mlflow_tracking_uri
...
def load_data(path, label_col, filter_expr, origin_col, origin_weights, origin_default_weight, positive_rate=None, test_split=None):
    """
...
def greedy_select(
    ...
    return selected_rules, selected_masks, prec_values, rec_values, num_tps, num_pos_preds
...
def main(cfg: DictConfig):
    ...
    with mlflow.start_run(run_name=f"MR:{MIN_RECALL} PR:{POSITIVE_RATE} MP:{MAX_PREDICATES_PER_RULE}", description=f"FN:{FILE_NAME}"):
        ...
        candidates = apriori_candidates(features, groups, labels, MAX_PREDICATES_PER_RULE, MIN_NEW_TP)
        ...
        selected_rules, selected_masks, prec_values, rec_values, num_tps, num_pos_preds = greedy_select(
            candidates,
            labels,
            weights,
            total_pos,
        ...
        if len(selected_rules_post_subsumed) < n0:
            logging.info(f"Subsumption: {n0} → {len(selected_rules_post_subsumed)} rules")
        ...
        if len(selected_rules_post_redundant) < n1:
            logging.info(f"Unique-TP:   {n1} → {len(selected_rules_post_redundant)} rules")
        ...
        for window_name, comp_dict in evaluation_dict.items():
            for comp_name, plot_dict  in comp_dict.items():
                ...
                mlflow.log_figure(
                    plot_dict["prec_rec"],
                    f"{prefix}/precision_recall.png"
                )
                mlflow.log_figure(
                    plot_dict["pr_value"],
                    f"{prefix}/pr_value_landscape.png"
                )
                mlflow.log_figure(
                    plot_dict["lar_heatmap"],
                    f"{prefix}/lar_heatmap.png"
                )
                mlflow.log_figure(
                    plot_dict["time_difference"],
                    f"{prefix}/time_difference.png"
                )
        ...
        mlflow.log_metrics(
            {
                ...
                "09_NUMER OF RULES POST REDUNDANT": n2,
                ...
                "13_2_OUT_STACK_TOTAL_POS":float(fw["out_of_stack_stats"]["total_pos"]),
                ...
                "BOTH PAID SUM OF MIN MAX_REIMB OF TPS":  gw["savings"]["both_paid_total"],
                ...
                "ALL_POS AVG SAVINGS (savings_per_tp est)": gw["savings_all_pos"]["average_savings"],
            }
        )
